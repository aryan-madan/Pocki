...
class Y extends MeshPhysicalMaterial {
  ...
  constructor(params: any) {
    super(params);
    // fix: Cast 'this' to any to bypass TypeScript errors for properties inherited from MeshPhysicalMaterial.
    ...
    (this as any).onBeforeCompile = (shader: any) => {
      ...
      // fix: Replace 'replaceAll' with 'split' and 'join' for broader TypeScript target compatibility.
  ...
  if (e.touches.length > 0) {
    ...
    for (const [elem, data] of pointerMap) {
      ...
      if (isInside(rect)) {
        ...
        // Only prevent default if the touch starts directly on the canvas.
        // This allows touches on elements layered on top (like buttons) to function correctly.
        if (target === elem) {
          e.preventDefault();
        }
        data.touching = true;
...
function onTouchMove(e: TouchEvent) {
  if (e.touches.length > 0) {
    // Check if any of our managed canvases are currently being touched.
    let isTouchingOurCanvas = false;
    for (const data of pointerMap.values()) {
        if (data.touching) {
            isTouchingOurCanvas = true;
            break;
        }
    }
    
    // If a drag is in progress on one of our canvases, prevent default browser behavior like scrolling.
    if (isTouchingOurCanvas) {
        e.preventDefault();
    }
...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof XConfig> = {}) {
    ...
    // fix: Cast material to any to access property inherited from Material.
  ...
  #setupLights() {
    ...
    // fix: Cast 'this' to any to bypass TypeScript error for 'add' method inherited from Object3D.
  ...
  setColors(colors: number[]) {
    if (Array.isArray(colors) && colors.length > 1) {
      ...
      // fix: Cast 'this' to any to access properties/methods from InstancedMesh due to typing issues.
